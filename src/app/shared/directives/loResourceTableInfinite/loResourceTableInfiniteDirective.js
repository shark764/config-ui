...
angular.module('liveopsConfigPanel')
   ...
   */
  .directive('loResourceTableInfinite', ['$rootScope', '$filter', '$location', '$parse', 'loEvents', 'DirtyForms', '$q',
    function($rootScope, $filter, $location, $parse, loEvents, DirtyForms, $q) {
      return {
        restrict: 'E',
        scope: {
        ...
        templateUrl: 'app/shared/directives/loResourceTableInfinite/loResourceTableInfinite.html',
        transclude: true,
        ...
        link: function($scope) {
          ...
          $q.when($scope.items.$promise, function() {
            filterItems();
          ...
          $rootScope.$on('filtersChanged', filterItems);
          function filterItems() {
            $scope.filteredItems = $filter('search')($scope.items, $scope.config.searchOn, $scope.searchQuery);
            $scope.filteredItems = $filter('orderBy')($scope.filteredItems, $scope.orderBy, $scope.reverseSortOrder);
            $scope.filteredItems = $filter('selectedTableOptions')($scope.filteredItems, $scope.config.fields);

            if ($scope.filteredItems) {
              $scope.renderedItems = $scope.filteredItems.slice(0,itemsShown);
            } else {
              $scope.renderedItems = [];
            }
          }
