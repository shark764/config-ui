        ...
        this.responseError = function(response) {
          $injector.invoke(function($state, $stateParams, $location, $document, AuthService, jwtHelper) {
            ...
            if (
              ...
              (response.status === 401 && Session.token && jwtHelper.isTokenExpired(Session.token))
            ) {
          ...
          if (
            Session.token &&
            Session.token.indexOf('Token') >= 0 &&
            response.status === 401 &&
            response.config.url !== apiHostname + '/v1/tenants/null/users'
          ) {
