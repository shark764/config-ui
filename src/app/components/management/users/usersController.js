...
angular.module('liveopsConfigPanel').controller('UsersController', [
  '$scope',
  '$translate',
  '$location',
  '$state',
  ...
  function(
    ...
    $q.all([IdpSvc.getActiveFilteredIdps(), $scope.tenantData]).then(function(response) {
    ...
    $scope.displaySsoFields = function() {
      return $scope.loadingIdps === false && $scope.loadingIdpList === false;
    };
    ...
    $scope.setInviteButton = function(tenantUserObj, btnType) {
      ...
      if (
        ...
        (tenantUserObj.noPassword !== cxEngageAuthStatesUser.enabled || $scope.tenantCxLoginStatus === 'enabled')
      ) {
        // if the user has username and password set, then show the "reset" button
        if (
          (tenantUserObj.invitationStatus === 'accepted' || tenantUserObj.invitationStatus === 'enabled') &&
          !$scope.savedCxStatus
        ) {
    ...
    function getIdentityProviders(tenantUserData) {
      ...
      if ($scope.tenantData.cxengageIdentityProvider) {
        $scope.tenantCxLoginStatus = $scope.tenantData.cxengageIdentityProvider;
        tenantDefaultVal =
          $scope.tenantData.cxengageIdentityProvider === cxEngageAuthStatesTenant.enabled
      ...
      if ($scope.tenantData.cxengageIdentityProvider === cxEngageAuthStatesTenant.denied) {
        ...
        (!Session.isSso && tenantUserData.id === Session.user.id)
      ) {
        ...
        tenantUserData.noPassword = noPassword ? cxEngageAuthStatesUser.disabled : cxEngageAuthStatesUser.enabled;
      }
    ...
    $scope.resetPassword = function() {
      return Modal.showConfirm({
        ...
        okCallback: function() {
          ResetPassword.initiateReset($scope.selectedTenantUser.id).then(
            function(response) {
              Alert.success($translate.instant('user.details.resetpassword.success'));
              // reset the SSO UI
              handleTenantUserSelect(
                _.merge($scope.selectedTenantUser, {
    ...
    $scope.isTenantSetForReadAllMode = function() {
    ...
    $scope.setUserAsImpersonated = function() {
      sessionStorage.setItem(
        'LOGI-USER-IMPERSONATE',
        JSON.stringify({
          id: $scope.selectedTenantUser.$user.id,
          displayName: $scope.selectedTenantUser.$original.getDisplay()
        })
      );
      ...
      return $scope.selectedTenantUser
        ...
        .then(
          function(tenantUser) {
            ...
            return tenantUser.save().then(
              function(tenantUserResponse) {
    ...
    $scope.canSaveUser = function(tenantUser) {
      return (
        $scope.scenario() !== 'invite:existing:user' &&
        ((tenantUser && tenantUser.$user && tenantUser.$user.isNew()) ||
          ...
          (UserPermissions.hasPermission('PLATFORM_MANAGE_USER_ACCOUNT') &&
            _.has($scope, 'selectedTenantUser.$user.id') &&
    ...
    $scope.submit = function() {
      ...
      if (
        !(angular.isFunction($scope.selectedTenantUser.isNew) && $scope.selectedTenantUser.isNew()) &&
        !$scope.selectedTenantUser.$user.externalId
      ) {
        $scope.selectedTenantUser.$user.externalId = null;
      }

      var userSave = $scope.canSaveUser($scope.selectedTenantUser) ? $scope.selectedTenantUser.$user.save : $q.when;

      var tenantUserSave = vm.canSaveTenantUser($scope.selectedTenantUser) ? vm.saveTenantUser : $q.when;

      return userSave.call($scope.selectedTenantUser.$user).then(tenantUserSave);
    };
    ...
    function handleTenantUserSelect(tenantUserData) {
      ...
      if (angular.isObject(tenantUserData) && !angular.isArray(tenantUserData)) {
        ...
        $q.when(tenantUserObj.$promise).then(
          function(tenantUser) {
    ...
    $scope.hasRequiredProps = function(tenantUserData) {
      ...
      return (
        ...
        (!tenantUserData.$original.$user.firstName || !tenantUserData.$original.$user.lastName)
      );
    };
    ...
    $scope.invitedUserExistsOnPlatform = function() {
      ...
      if ($scope.selectedTenantUser.$user.created || !$scope.selectedTenantUser.isNew()) {
